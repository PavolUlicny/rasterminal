...
ParseResult parse_args(int argc, char *argv[])
{
    ...
    auto print_help = [prog]()
    {
        std::printf(
            ...
            "\n"
            "Render a 3D model in the terminal: as real pixels where the terminal\n"
            "supports kitty or sixel graphics, else as unicode half-block cells\n"
            "(see --graphics), in 24-bit color with a 256-color fallback.\n"
            "\n"
            "Supported formats:\n"
            ...
            "  -j [N], --threads [N]          Worker threads (default: all cores under kitty and\n"
            "                                  sixel, min(cores,4) under half-blocks)\n"
