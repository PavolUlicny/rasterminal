    ...
    class InputController
    {
      public:
        ...
        )
        {
            ...
            if (!enabled_ || ev.type == terminal_input::InputEvent::Type::None ||
                ev.type == terminal_input::InputEvent::Type::CellSize ||
                ev.type == terminal_input::InputEvent::Type::SixelGeometry)
            {
                return false;
            }
            Camera &camera = state.camera;
            if (ev.type == terminal_input::InputEvent::Type::Key)
            {
                ViewerSettings &settings = state.settings;
                const terminal_input::Key k = ev.key;
                switch (k)
                {
                case terminal_input::Key::Space:
                    settings.spinning = !settings.spinning;
                    break;
                case terminal_input::Key::Num1:
                    settings.shading = ShadingMode::Wireframe;
                    break;
                case terminal_input::Key::Num2:
                    settings.shading = ShadingMode::Flat;
                    break;
                case terminal_input::Key::Num3:
                    settings.shading = ShadingMode::Phong;
                    break;
                case terminal_input::Key::B:
                    settings.background = cycle(settings.background, BACKGROUND_COUNT);
                    break;
                case terminal_input::Key::L:
                    settings.lighting = cycle(settings.lighting, LIGHTING_MODE_COUNT);
                    break;
                case terminal_input::Key::C:
                    settings.wireframe_color = cycle(settings.wireframe_color, WIREFRAME_COLOR_COUNT);
                    break;
                case terminal_input::Key::K:
                    settings.culling = !settings.culling;
                    break;
                case terminal_input::Key::T:
                    if (has_textures_)
                    {
                        settings.texturing = !settings.texturing;
                    }
                    break;
                case terminal_input::Key::R:
            ...
            else if (ev.type == terminal_input::InputEvent::Type::ScrollUp)
            {
            ...
            else if (ev.type == terminal_input::InputEvent::Type::ScrollDown)
            {
                if (camera.first_person)
                {
            ...
            else if (ev.type == terminal_input::InputEvent::Type::MousePress)
            {
                mouse_last_x_ = ev.x;
                mouse_last_y_ = ev.y;
                mouse_dragging_ = true;
            }
            else if (ev.type == terminal_input::InputEvent::Type::MouseRelease)
            {
                // Button identity is unknown; any release ends the drag.
                mouse_dragging_ = false;
            }
            else if (ev.type == terminal_input::InputEvent::Type::MouseMove)
            {
        ...
        {
            if (held_cam_key_ == terminal_input::Key::None)
            {
                return false;
            }
      ...
      private:
        ...
        bool enabled_;
        bool has_textures_;
        terminal_input::Key held_cam_key_ = terminal_input::Key::None;
