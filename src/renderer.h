...
struct Renderer
{
    ...
    //
    // cppcheck's unusedFunction is whole-program over src/*.cpp only, so a member whose
    // sole caller is a test always reads as unused. Suppressed here rather than by adding
    // src/renderer.h to the suppressions file, which would mask every future one too.
