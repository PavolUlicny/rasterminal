...
struct Renderer
{
    ...
    // Forced paths are test hooks; Auto chooses by frame geometry.
    ...
    // Run once per worker and block. The pool is not reentrant; tasks report errors
    // through caller-owned state because worker std::exceptions are swallowed.
    ...
    // Test hook for the path selected by Auto.
  ...
  private:
    ...
    // Rasterize stolen triangle chunks. Sink and shading branches compile out.
    template <Sink S, ShadingMode M> void raster_triangles(int worker_id);

    // Select the compile-time shading instantiation.
    template <Sink S> void dispatch_raster(int worker_id);

    // Bin on-screen triangles, then steal tiles for visibility and deferred shading.
    ...
    // Bounds and target band for transparent work-steal tuning.
    ...
    // Screen triangle, source attributes, and inclusive tile bounds.
    struct RasterTri
    {
    ...
    // Workers steal triangles; draw_line's depth CAS keeps the result order-independent.
    void raster_wireframe();

    // Composite fragment lists back-to-front over the opaque framebuffer.
    void resolve_pixels();

    // Resize and clear A-buffer heads when the framebuffer changes.
