...
| `--graphics-scale` | none | `0.75` | Render scale for the kitty backend in `[0.05, 1]`: the frame renders at this fraction of the window's pixel resolution and the terminal stretches it back over the same cells, trading sharpness for speed; `1` = native; an error with an explicit `--graphics blocks`, inert when `auto` falls back to blocks |
...
- Pixel rendering on terminals implementing the kitty graphics protocol, detected by a startup query and overridable with `--graphics`; renders at 0.75 of the window resolution by default (`--graphics-scale 1` for native); not available under tmux or GNU screen (no passthrough support)
