...
// Dense overlapping faces create CAS contention across work claims. Add off-screen,
// near-clipped, culled, and double-sided triangles to cover every wireframe edge path.
...
TEST(renderer, wireframe_multiworker_matches_singleworker)
{
    ...
    // Ensure the comparison did not pass on two blank framebuffers.
...
// A large triangle spans every worker band, exposing torn or corrupted colour writes.
TEST(renderer, wireframe_multiworker_color_uniform)
{
    ...
    // Detect coverage through depth so a corrupted black pixel cannot disappear from the check.
    int drawn = 0;
    for (int y = 0; y < fb.height(); y++)
    {
        for (int x = 0; x < fb.width(); x++)
...
// Soft-knee tonemapping keeps an untextured surface lit to 1.4 below white, preserving headroom.
TEST(renderer, untextured_overbright_phong_rolls_off_below_white)
{
    ...
    ASSERT_TRUE(was_drawn(fb, 20, 10));
    Color c = fb.get_pixel(20, 10);
    // Lit to 1.4: a hard clamp gives 255, while the rolloff stays below it.
    ...
    if (c.r < 235) // still bright without clipping
    {
...
// A zero emissive factor must keep a bound bright texture dark. Force has_emissive
// so only the rasterizer's do_emissive gate can suppress the forwarded texture.
...
// Flat and Phong have independent emissive gates, so both need the zero-factor case.
...
// Hiding textures must not suppress an authored emissive factor.
...
// With a bound map, hiding textures still preserves the authored factor.
...
// With textures hidden, a bound black specular map and normal map must not affect output.
// Camera and light at +Z produce peak specular when those maps are absent.
...
// Flip backface normals from -Z to +Z so the +Z light contributes.
...
// Flat shading has a separate face-normal negation branch.
...
// With culling off, the -Z backface normal stays unflipped and receives no +Z light.
...
// A front-facing double-sided triangle keeps its +Z normal and receives full +Z light.
