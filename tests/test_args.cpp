...
TEST(args, graphics_scale_default)
{
    ASSERT_NEAR(run({ "m.obj" }).args.graphics_scale, 0.75f, 1e-6f);
}

TEST(args, graphics_scale_sets_value)
{
    ParseResult r = run({ "--graphics-scale", "0.5", "m.obj" });
    ASSERT_TRUE(r.ok);
    ASSERT_NEAR(r.args.graphics_scale, 0.5f, 1e-6f);
}

TEST(args, graphics_scale_equals_form)
{
    ParseResult r = run({ "--graphics-scale=0.33", "m.obj" });
    ASSERT_TRUE(r.ok);
    ASSERT_NEAR(r.args.graphics_scale, 0.33f, 1e-6f);
}

TEST(args, graphics_scale_accepts_range_bounds)
{
    ASSERT_TRUE(run({ "--graphics-scale=0.05", "m.obj" }).ok);
    ASSERT_TRUE(run({ "--graphics-scale=1", "m.obj" }).ok);
}

TEST(args, graphics_scale_out_of_range_is_error)
{
    ASSERT_FALSE(run({ "--graphics-scale=0.04", "m.obj" }).ok);
    ASSERT_FALSE(run({ "--graphics-scale=1.5", "m.obj" }).ok);
    ASSERT_FALSE(run({ "--graphics-scale=0", "m.obj" }).ok);
    ASSERT_FALSE(run({ "--graphics-scale=-0.5", "m.obj" }).ok);
}

TEST(args, graphics_scale_rejects_non_numeric)
{
    ParseResult r = run({ "--graphics-scale", "half", "m.obj" });
    ASSERT_FALSE(r.ok);
    ASSERT_EQ(r.exit_code, 1);
}

TEST(args, graphics_scale_rejects_non_finite)
{
    ASSERT_FALSE(run({ "--graphics-scale=nan", "m.obj" }).ok);
    ASSERT_FALSE(run({ "--graphics-scale=inf", "m.obj" }).ok);
}

TEST(args, graphics_scale_missing_value_is_error)
{
    ParseResult r = run({ "--graphics-scale" });
    ASSERT_FALSE(r.ok);
    ASSERT_EQ(r.exit_code, 1);
}

TEST(args, graphics_scale_rejected_with_explicit_blocks)
{
    // Under an explicit --graphics blocks the scale can never come into play; the
    // flag is rejected rather than silently ignored (the --first-person-speed rule).
    ParseResult r = run({ "--graphics", "blocks", "--graphics-scale=0.5", "m.obj" });
    ASSERT_FALSE(r.ok);
    ASSERT_EQ(r.exit_code, 1);
}

TEST(args, graphics_scale_blocks_check_order_does_not_matter)
{
    // The check runs after the whole command line is parsed.
...
{
    // Auto may resolve to kitty at runtime, so the scale can come into play there.
    ParseResult k = run({ "--graphics", "kitty", "--graphics-scale=0.5", "m.obj" });
    ASSERT_TRUE(k.ok);
    ASSERT_NEAR(k.args.graphics_scale, 0.5f, 1e-6f);
    ASSERT_TRUE(run({ "--graphics-scale=0.5", "m.obj" }).ok);
}
